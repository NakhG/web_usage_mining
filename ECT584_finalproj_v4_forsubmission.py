...
plt.hist(dataiku_userlogs['time_spent'], bins=20)
plt.title('Distribution of time spent per page (seconds)')
...
dataiku_userlogs['time_spent'] = dataiku_userlogs['time_spent'].astype(float)
...
plt.hist(clickstream_df['session_total_time'], bins=30)
plt.title('Session lengths (seconds)')

clickstream_df['session_total_pages'].mean() #2.7 pages
plt.hist(clickstream_df['session_total_pages'], bins=30)
plt.title('Session lengths (pages visited)')
...
clickstream_df['browser'].value_counts().plot(kind = 'bar')


clickstream_df['os'].value_counts() #Chrome by far. Firefox next.
clickstream_df['os'].value_counts().plot(kind = 'bar')



100. * clickstream_df['country'].value_counts() / len(clickstream_df['country'])  #United State by far. Interesting that French is so much smaller, given it is a French company. 
clickstream_df['country'].value_counts().plot(kind='bar')
...
100. * landing_pgs.value_counts() / len(landing_pgs) #home page by far, followed by products, and blog posts
...
100. * pd.Series(landing_cats).value_counts() / len(pd.Series(landing_cats))  #home still leads, but blog posts not very far behind: may imply marketing success
